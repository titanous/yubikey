...
describe 'Yubikey::OTP::Verify' do
  ...
  it 'should verify a valid OTP' do
    ...
    otp = Yubikey::OTP::Verify.new(:id => @id, :key => @key, :otp => @otp, :nonce => @nonce)
  ...
  it 'should raise on invalid OTP' do
    ...
    @mock_http_get.should_receive(:body).and_return("h=#{hmac}\n#{bad_response}")
    lambda { otp = Yubikey::OTP::Verify.new(:id => @id, :key => @key, :otp => @otp, :nonce => @nonce) }.should raise_error(Yubikey::OTP::InvalidOTPError)
  end
